...
def CalcMatrix(sid,eid,inPath,outPath,ScenePrefix, tsid):
    ...
    for i in range(sid,eid):
        ...
        #print("{}/{}.{}.txt".format(inPath,ScenePrefix,prev_idx1))
        with open("{}/{}.{}.txt".format(inPath,ScenePrefix,prev_idx1),'r') as prev_file1:
            ...
            matches = re.findall(r'-?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?', lines[2])
            #print(len(matches),matches,lines[1])
            assert len(matches)==16
            rv1=np.array([float(matches[0]),float(matches[1]),float(matches[2]),float(matches[3])])
            uv1=np.array([float(matches[4]),float(matches[5]),float(matches[6]),float(matches[7])])
            fv1=np.array([float(matches[8]),float(matches[9]),float(matches[10]),float(matches[11])])
            wv1=np.array([float(matches[12]),float(matches[13]),float(matches[14]),float(matches[15])])
            ProjMartrix = np.array([
                    [rv1[0],uv1[0],fv1[0],wv1[0]],
                    [rv1[1],uv1[1],fv1[1],wv1[1]],
                    [rv1[2],uv1[2],fv1[2],wv1[2]],
                    [rv1[3],uv1[3],fv1[3],wv1[3]],
            ]).transpose()
            ...
            with open("{}/{}.{}.txt".format(inPath,ScenePrefix,prev_idx3),'r') as prev_file3:
                ...
                matches = re.findall(r'-?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?', lines[7])
                assert len(matches)==3
                ViewRotation1=np.array([float(matches[0]),float(matches[1]),float(matches[2])])
                ...
                #print(idx,ViewLocation,ViewRotation) Debug
                q0=Euler2Quat(pitch=ViewRotation1[0],yaw=ViewRotation1[1],roll=ViewRotation1[2],isDegree=True)
                alpha=(i%4)/4
                ...
                save_path = "{}/{}".format(outPath,ScenePrefix)
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                with open("{}/{}.{}.txt".format(save_path,ScenePrefix,idx),'w') as file:
                    file.write("\nOurs Inter ViewMatrix:"+str(ViewMatrix))
                    file.write("\nOurs Inter ProjMatrix:"+str(ProjMartrix))
...
if __name__ == "__main__":
    ...
    sequence_names = ["MedievalDocks/test1-60fps"]
